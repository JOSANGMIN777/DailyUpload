...
# 우선 문제 이해를 잘못해서 아니 왜 1이랑 6의 부모의 출력을 4라고 하지?? 1이 루트노드면 1이랑 연결된 노드들의 부모는 당연히 다 1 아니야? 
# 이러고 있었음 근데 사실은 그게 아니라 1번 노드를 제외한 2번 노드부터의 바로 위 부모를 나타낸거기에 그런 출력값이 나온거였다...
...
# 1이랑 바로 연결된 노드가 나중에 if visited[v] = 0 조건 때문에 덮어쓰기함
# 걍 1번의 부모는 자기자신이라고 하는게 오히려 이해하기 쉬울듯
# 마지막에 출력을 저렇게 한것은 for문 보다 메모리와 시간 측면에서 아주 약간이라도 더 절약할 수 있어서 그렇다.